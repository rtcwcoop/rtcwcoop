...
qhandle_t R_RegisterMD3(const char *name, model_t *mod)
{
	...
	for (lod = MD3_MAX_LODS - 1 ; lod >= 0 ; lod--)
	{
		...
		// there's an mdc model in mp_pakmaps0.pk3 that crashes the renderer
		// there also exists an md3 model of it, so we'll ignore that mdc file and load the md3 instead
		if ( !strcmp( namebuf, "models/mapobjects/tree/tree_m08.md3" ) ||
			 !strcmp( namebuf, "models/mapobjects/tree/tree_m07.md3" ) ) {
			ignore = qtrue;
		}
