...
void CL_ConnectionlessPacket( netadr_t from, msg_t *msg ) {
	...
	if (!Q_stricmp(c, "challengeResponse"))
	{
		...
		if ( Cmd_Argc() > 4 ) {
			clc.onlyVisibleClients = atoi( Cmd_Argv( 4 ) );         // DHM - Nerve
		} else {
...
void CL_Frame( int msec ) {
	...
	// Allow timescale only when in demo..
	// When multiple clients set their timescale to
	// a high value and then simultaneously connect to a server it will crash
#ifndef _DEBUG
	if ( !clc.demoplaying ) {
...
void CL_InitRef( void ) {
	...
	Com_sprintf(dllName, sizeof(dllName), "renderer_coop_%s_" ARCH_STRING DLL_EXT, cl_renderer->string);
	...
	if(!(rendererLib = Sys_LoadDll(dllName, qfalse)) && strcmp(cl_renderer->string, cl_renderer->resetString))
	{
		...
		Com_sprintf(dllName, sizeof(dllName), "renderer_coop_opengl1_" ARCH_STRING DLL_EXT);
...
void CL_Init( void ) {
	...
	cl_master = Cvar_Get( "cl_master", "master.rtcwcoop.com", CVAR_ARCHIVE );

	// people got the ip of the masterserver in their saved configs, lets change that to master.rtcwcoop.com
	// so in case we ever change the ip of master.rtcwcoop.com they still can see the master
	// we can remove this in a few releases
	if ( !Q_stricmp( cl_master->string, "78.47.152.4" ) || !Q_stricmp( cl_master->string, "88.159.160.153" ) ) {
		Cvar_Set( "cl_master", "master.rtcwcoop.com" );
	}
...
void CL_GetLatestVersion_f( void ) {
	...
	NET_StringToAdr( cl_master->string, &to, NA_UNSPEC );
