...
ifeq ($(PLATFORM),linux)
  ...
  BASE_CFLAGS = -Wall -fno-strict-aliasing -Wimplicit \
    -pipe -DUSE_ICON -DC_ONLY -fno-common -D_ADMINS -DMONEY
...
ifeq ($(PLATFORM),darwin)
  ...
  BASE_CFLAGS = -Wall -Wimplicit -DC_ONLY -m32 -D_ADMINS -DMONEY
...
debug:
	@$(MAKE) targets B=$(BD) CFLAGS="$(CFLAGS) $(BASE_CFLAGS) $(DEPEND_CFLAGS)" \
	  OPTIMIZE="-D_DEBUG $(DEBUG_CFLAGS)" OPTIMIZEVM="$(DEBUG_CFLAGS)" \
