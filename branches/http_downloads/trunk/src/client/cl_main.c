...
void CL_DisconnectPacket( netadr_t from ) {
	...
	*/
	...
	if ( !cls.bWWWDlDisconnected ) {
// Is this really need it? Can't we just go directly with localisation on...?
#ifdef LOCALISATION
		// drop the connection
		message = CL_TranslateStringBuf( "Server disconnected for unknown reason\n" );
		Com_Printf( message );
...
void CL_WWWDownload( void ) {
	...
	if ( ret == DL_DONE ) {
		...
		if ( rename( cls.downloadTempName, to_ospath ) ) {
			// L0 - FS_CopyFileOS should do the magic..or am i missing smth and we should go with FS_CopyFile?
			FS_CopyFileOS( cls.downloadTempName, to_ospath );
...
// JPW NERVE
	...
	cls.consoleShader = re.RegisterShader( "console" ); // JPW NERVE shader works with 16bit
	cls.consoleShader2 = re.RegisterShader( "console2" ); // JPW NERVE same
...
void CL_Init( void ) {
	...
	cl_master = Cvar_Get( "cl_master", "master.rtcwcoop.com", CVAR_ARCHIVE );
        ...
        // fretn: people got the ip of the masterserver in their saved configs, lets change that to master.rtcwcoop.com
